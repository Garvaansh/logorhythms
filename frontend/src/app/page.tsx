...
import { ArrowRight, ArrowUpRight, GitBranch, Sparkles, Play, Terminal, Mic, Layers, Braces, Shield, BarChart3, Zap, Code2, Github, Globe, Headphones, FileCode, Check, ChevronDown, Users, BookOpen, Workflow, MessageSquare } from 'lucide-react'
    ...
    <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden selection:bg-indigo-500/30">
      ...
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${headerScrolled ? 'bg-[#050505]/80 backdrop-blur-2xl border-b border-white/[0.06]' : 'bg-transparent'}`}>
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <button key={item} className="text-[13px] tracking-wide uppercase text-white/30 hover:text-white transition-colors duration-300"
                onClick={() => { document.getElementById(item.toLowerCase().replace(/['\s]+/g, '-'))?.scrollIntoView({ behavior: 'smooth' }) }}>
                {item}
              </button>
            ))}
            <Link href="/mcp-guide" className="text-[13px] tracking-wide uppercase text-white/30 hover:text-white transition-colors duration-300 flex items-center gap-1.5">
              <Terminal className="w-3.5 h-3.5" /> IDE
            </Link>
            <Link href="/pricing" className="text-[13px] tracking-wide uppercase text-white/30 hover:text-white transition-colors duration-300">Pricing</Link>
          </nav>
          <div className="flex items-center gap-4">
            <Link href={authTarget} onClick={handleAuthClick} className="text-[13px] text-white/40 hover:text-white transition-colors hidden sm:block">
              {isAuthenticated ? 'Dashboard' : 'Login'}
            </Link>
            <Link href={authTarget} onClick={handleAuthClick}
              className="text-[13px] font-medium text-white/40 hover:text-white transition-colors flex items-center gap-1">
      ...
      <section className="relative min-h-screen flex flex-col justify-center pt-16 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          <motion.div className="absolute top-[-20%] left-[40%] w-[1.5px] h-[150vh] bg-gradient-to-b from-transparent via-white/[0.6] to-transparent shadow-[0_0_20px_rgba(255,255,255,0.4)]" 
            style={{ transform: 'rotate(35deg)', transformOrigin: 'top center' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2, ease: 'easeOut' }} />
          <motion.div className="absolute top-[-20%] right-[40%] w-[1.5px] h-[150vh] bg-gradient-to-b from-transparent via-white/[0.5] to-transparent shadow-[0_0_20px_rgba(255,255,255,0.3)]" 
            style={{ transform: 'rotate(-35deg)', transformOrigin: 'top center' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2, ease: 'easeOut', delay: 0.2 }} />

          {/* Far left diagonal */}
          <motion.div className="absolute top-[-10%] left-[10%] w-[1px] h-[120vh] bg-gradient-to-b from-transparent via-white/[0.4] to-transparent shadow-[0_0_15px_rgba(255,255,255,0.2)]" 
            style={{ transform: 'rotate(25deg)' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2, ease: 'easeOut', delay: 0.4 }} />

          {/* Far right sharp diagonal */}
          <motion.div className="absolute top-[10%] right-[-10%] w-[2px] h-[100vh] bg-gradient-to-b from-transparent via-white/[0.3] to-transparent shadow-[0_0_15px_rgba(255,255,255,0.2)]" 
            style={{ transform: 'rotate(-55deg)' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2, ease: 'easeOut', delay: 0.6 }} />

          {/* Horizontal cross beam */}
          <motion.div className="absolute top-[40%] left-[-10%] w-[1px] h-[120vw] bg-gradient-to-b from-transparent via-white/[0.2] to-transparent shadow-[0_0_10px_rgba(255,255,255,0.1)]" 
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 sm:px-10 w-full">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-4 items-center">
            ...
            <div className="max-w-2xl">
              <h1
                className="text-[clamp(3rem,9vw,7.5rem)] font-bold tracking-[-0.06em] leading-[0.85] mb-8 text-white/90 whitespace-nowrap"
              ...
              <motion.p
                className="text-[15px] sm:text-base text-white/35 leading-relaxed max-w-sm mb-10"
              ...
              >
                <Link href={authTarget} onClick={handleAuthClick}
                  className="group inline-flex items-center gap-3 bg-white text-black font-semibold text-[13px] tracking-[0.15em] uppercase px-8 py-4 hover:bg-white/90 active:scale-[0.97] transition-all duration-300">
                  Get Started Now
      ...
      {/* ─── MARQUEE ─── */}
      <div className="relative py-8 border-y border-white/[0.04] overflow-hidden bg-[#050505]">
            ...
            <div key={outer} className="flex gap-12 items-center shrink-0 marquee-content">
              {['Python','TypeScript','React','Go','Rust','Java','Next.js','Node.js','Ruby','Kotlin','Swift','C++'].map((lang) => (
                <span key={`${outer}-${lang}`} className="flex items-center gap-3">
                  ...
                  <span className="text-[13px] font-mono tracking-wide text-white/20 uppercase">{lang}</span>
                </span>
              ))}
            </div>
          ))}
      ...
      <section className="relative py-24 sm:py-32 bg-[#050505]">
        <div className="max-w-[1100px] mx-auto px-6 sm:px-10">
          <motion.div className="text-center mb-14"
            ...
            <span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/30 mb-4 font-medium">
              <span className="w-8 h-[1px] bg-white/20" /> See it in action
            </span>
            <h2 className="text-[clamp(1.5rem,4vw,2.5rem)] font-bold tracking-[-0.03em] text-white/85">
          ...
          <motion.div className="relative"
            ...
            {/* Browser mockup */}
            <div className="relative rounded-2xl overflow-hidden bg-[#0a0a0f] border border-white/[0.08] shadow-[0_30px_80px_rgba(0,0,0,0.4)]">
              {/* Chrome bar */}
              ...
              <div className="font-mono text-[12.5px] leading-[2] p-5 sm:p-6">
                {[
                  ...
                  <motion.div key={l.n}
                    ...
                    <span className="w-7 text-right text-[11px] text-white/10 select-none pr-3 shrink-0 font-mono">{l.n}</span>
              ...
              <div className="flex items-center gap-3 px-5 py-3 bg-white/[0.02] border-t border-white/[0.06]">
                ...
                <span className="text-[10px] text-white/20 font-mono tabular-nums shrink-0">1:24</span>
              </div>
            </div>

            {/* Floating feature badges */}
            <motion.div className="absolute -top-4 -right-4 sm:-right-8 bg-[#111116] border border-white/[0.08] rounded-xl px-4 py-3 shadow-[0_8px_30px_rgba(0,0,0,0.3)]"
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: false }} transition={{ delay: 0.6, duration: 0.6, ease }}
              animate={{ y: [0, -6, 0] }} >
              <div className="flex items-center gap-2">
                <Mic className="w-4 h-4 text-indigo-500" />
                <span className="text-[12px] font-semibold text-white/60">AI Narration Active</span>
              </div>
            </motion.div>

            <motion.div className="absolute -bottom-4 -left-4 sm:-left-8 bg-[#111116] border border-white/[0.08] rounded-xl px-4 py-3 shadow-[0_8px_30px_rgba(0,0,0,0.3)]"
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: false }} transition={{ delay: 0.8, duration: 0.6, ease }}
              animate={{ y: [0, -4, 0] }} >
              <div className="flex items-center gap-2">
                <Layers className="w-4 h-4 text-purple-500" />
                <span className="text-[12px] font-semibold text-white/60">Architecture Mapped</span>
              </div>
            </motion.div>
      ...
      <section id="what's-inside" className="relative py-32 sm:py-40 bg-[#050505]">
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10">
          <div className="mb-16 sm:mb-24">
            <motion.span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/30 mb-5 font-medium"
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }}>
              <span className="w-8 h-[1px] bg-white/20" /> What&apos;s Inside
            </motion.span>
            <motion.h2 className="text-[clamp(2rem,5vw,3.8rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white/85"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={feat.title}
                className="group relative bg-[#0a0a0f] border border-white/[0.08] p-8 hover:border-white/[0.16] transition-all duration-500"
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: false, margin: '-40px' }} transition={{ duration: 0.7, ease, delay: i * 0.08 }}>
                <div className="w-12 h-12 rounded-xl bg-white/[0.04] flex items-center justify-center mb-6 text-white/60 group-hover:bg-white group-hover:text-black transition-all duration-500">
                  {feat.icon}
                </div>
                <h3 className="text-lg font-bold tracking-tight mb-3 text-white/90">{feat.title}</h3>
                <p className="text-[14px] text-white/40 leading-relaxed">{feat.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ─── HOW IT WORKS ─── */}
      <section id="how-it-works" className="relative py-32 sm:py-40 bg-[#050505] text-white">
        <div className="max-w-[1100px] mx-auto px-6 sm:px-10">
          <div className="mb-20 sm:mb-28">
            <motion.span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/30 mb-5 font-medium"
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }}>
              <span className="w-8 h-[1px] bg-white/20" /> How It Works
            </motion.span>
            <motion.h2 className="text-[clamp(2rem,5vw,3.8rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white/90"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, ease }}>
            ...
            {[
              ...
              <motion.div key={step.num} className="flex gap-8 sm:gap-12 items-start"
                ...
                <div className="shrink-0">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 border border-white/[0.1] flex items-center justify-center text-white/60">
                    {step.icon}
                  </div>
                </div>
                <div className="pt-1">
                  <span className="text-[12px] font-mono tracking-wider mb-3 block text-white/25">STEP {step.num}</span>
                  <h3 className="text-[clamp(1.3rem,3vw,2rem)] font-bold tracking-[-0.02em] mb-3 text-white/90">{step.title}</h3>
                  <p className="text-[15px] sm:text-base text-white/30 leading-relaxed max-w-lg">{step.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* ─── STATS ─── */}
      <section className="relative py-24 sm:py-32 bg-[#0a0a0f] border-y border-white/[0.04]">
            ...
            {[
              ...
              return (
                <motion.div key={stat.label} className="text-center lg:text-left"
                  ...
                  <div className="flex items-center justify-center lg:justify-start gap-2 mb-3 text-white/30">{stat.icon}</div>
                  <span ref={ref} className="text-[clamp(2rem,5vw,3.5rem)] font-bold tracking-[-0.03em] text-white/90">
                    {count.toLocaleString()}{stat.suffix}
                  </span>
                  <p className="text-[13px] text-white/30 mt-1 tracking-wide uppercase">{stat.label}</p>
                </motion.div>
              )
            })}
          </div>
      ...
      <section className="relative py-28 sm:py-36 bg-[#050505]">
        <div className="max-w-[1200px] mx-auto px-6 sm:px-10">
          <div className="mb-16 sm:mb-20">
            <motion.span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/30 mb-5 font-medium"
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }}>
              <span className="w-8 h-[1px] bg-white/20" /> Why DocuVerse
            </motion.span>
            <motion.h2 className="text-[clamp(2rem,5vw,3.5rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white/85"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, ease }}>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {[
              ...
              <motion.div key={item.title}
                className="group relative bg-[#0a0a0f] border border-white/[0.08] p-8 sm:p-10 hover:border-white/[0.16] transition-all duration-500"
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: false, margin: '-40px' }} transition={{ duration: 0.7, ease, delay: i * 0.1 }}>
                <div className="flex items-start gap-5">
                  <div className="w-11 h-11 rounded-lg bg-white/[0.04] flex items-center justify-center shrink-0 text-white/50 group-hover:bg-white group-hover:text-black transition-all duration-500">
                    {item.icon}
                  </div>
                  <div>
                    <h3 className="text-lg font-bold tracking-tight mb-2 text-white/90">{item.title}</h3>
      ...
      {/* ─── TESTIMONIALS ─── */}
      <section className="relative py-28 sm:py-36 bg-black text-white overflow-hidden">
        <div className="max-w-[1200px] mx-auto px-6 sm:px-10">
          <div className="mb-16 sm:mb-20">
            <motion.span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/25 mb-5 font-medium"
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }}>
              <span className="w-8 h-[1px] bg-white/15" /> What developers say
            </motion.span>
            <motion.h2 className="text-[clamp(2rem,5vw,3.5rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white/90"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, ease }}>
              Trusted by builders
            </motion.h2>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={t.name}
                className="relative border border-white/[0.08] p-7 sm:p-8"
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: false, margin: '-40px' }} transition={{ duration: 0.7, ease, delay: i * 0.12 }}>
                <MessageSquare className="w-5 h-5 text-white/15 mb-5" />
                <p className="text-[15px] text-white/50 leading-relaxed mb-8">&ldquo;{t.quote}&rdquo;</p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-white/[0.08] flex items-center justify-center text-[13px] font-bold text-white/40">
                    {t.avatar}
                  </div>
                  <div>
                    <div className="text-[13px] font-semibold text-white/70">{t.name}</div>
                    <div className="text-[12px] text-white/25">{t.role}</div>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="relative py-28 sm:py-36 bg-[#050505]">
        <div className="max-w-[800px] mx-auto px-6 sm:px-10">
          <div className="mb-14 sm:mb-20 text-center">
            <motion.span className="inline-flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-white/30 mb-5 font-medium"
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }}>
              <span className="w-8 h-[1px] bg-white/20" /> FAQ
            </motion.span>
            <motion.h2 className="text-[clamp(2rem,5vw,3rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white/85"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, ease }}>
              Common questions
            </motion.h2>
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <motion.div key={i}
                className="border border-white/[0.06] bg-[#0a0a0f] overflow-hidden"
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: false, margin: '-40px' }} transition={{ duration: 0.5, ease, delay: i * 0.05 }}>
                <button
                  className="w-full flex items-center justify-between px-6 py-5 text-left group"
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  <span className="text-[15px] font-semibold text-white/80 group-hover:text-white transition-colors pr-4">{faq.q}</span>
                  <motion.div
                ...
                <AnimatePresence>
                  {openFaq === i && (
                    <motion.div
                      ...
                      transition={{ duration: 0.3, ease: [0.23, 1, 0.32, 1] }}>
                      <div className="px-6 pb-5 text-[14px] text-white/40 leading-relaxed border-t border-white/[0.04] pt-4">
                        {faq.a}
                      </div>
                    </motion.div>
                  )}
      ...
      {/* ─── CTA ─── */}
      <section className="relative py-40 sm:py-52 bg-[#0a0a0f] overflow-hidden">
        ...
        <motion.div className="relative z-10 max-w-2xl mx-auto text-center px-6"
          initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: false }} transition={{ duration: 0.8, ease }}>
          <h2 className="text-[clamp(2.2rem,6vw,4.5rem)] font-bold tracking-[-0.04em] leading-[0.95] mb-6 text-white/90">
            Start building<br />understanding
          </h2>
          <p className="text-base sm:text-lg text-white/40 mb-12 leading-relaxed max-w-md mx-auto">
            Connect a repo. Get your first walkthrough in under 60 seconds. No credit card required.
          </p>
          <Link href="/auth/signin"
            className="group inline-flex items-center gap-3 bg-white text-black font-semibold text-[13px] tracking-[0.15em] uppercase px-10 py-5 hover:bg-white/90 active:scale-[0.97] transition-all duration-300">
            Get Started Free
      ...
      {/* ─── FOOTER ─── */}
      <footer className="border-t border-white/[0.06] py-12 sm:py-16 bg-[#050505]">
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
            <div>
              <Link href="/" className="flex items-center gap-2 mb-3">
                <img src="/logo.png" alt="DocuVerse" className="w-6 h-6 rounded-md object-cover" />
                <span className="text-[14px] font-semibold text-white/50">DocuVerse</span>
              </Link>
              <p className="text-[13px] text-white/30 max-w-xs">Transform complex codebases into interactive, audio & visual walkthroughs.</p>
            </div>
            <div className="flex gap-10">
              <div className="space-y-3">
                <span className="text-[11px] tracking-[0.15em] uppercase text-white/20 font-medium">Product</span>
                <div className="space-y-2">
                  <Link href="/demo" className="block text-[13px] text-white/40 hover:text-white/80 transition-colors">Demo</Link>
                  <Link href="/mcp-guide" className="block text-[13px] text-white/40 hover:text-white/80 transition-colors">IDE Plugin</Link>
                </div>
              </div>
              <div className="space-y-3">
                <span className="text-[11px] tracking-[0.15em] uppercase text-white/20 font-medium">Company</span>
                <div className="space-y-2">
                  <Link href="/auth/signin" className="block text-[13px] text-white/40 hover:text-white/80 transition-colors">Sign in</Link>
                  <span className="block text-[13px] text-white/40">Team BitMask</span>
                </div>
              </div>
            </div>
          </div>
          <div className="mt-12 pt-6 border-t border-white/[0.04] flex items-center justify-between">
            <span className="text-[12px] text-white/20">© 2026 DocuVerse</span>
            <span className="flex items-center gap-1.5 text-[12px] text-white/20">Built with love by Logorhythms AI</span>
          </div>
        </div>
      </footer>
    </div>
